...
const getAvatarStyle = (code: string) => {
  const styles = [
    ...
    "bg-slate-100 text-foreground border border-slate-300",
  ];
...
export default function ProjectDevelopersPage() {
  ...
  // ── Query ──
  const filters: ProjectDeveloperFilters = useMemo(
    () => ({
  ...
  const { data, isLoading, isFetching, isError, refetch } = useQuery({
    ...
    queryFn: () => ProjectDeveloperService.listProjectDevelopers(filters),
    staleTime: 30_000,
  });

  const owners: ProjectDeveloperRecord[] = data?.data ?? [];
  ...
  return (
    <div className="animate-in fade-in duration-700 pb-24">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div>
              ...
              <h1 className="text-4xl md:text-5xl font-sans text-foreground tracking-tight leading-none mb-4">
                Project <span className="italic text-muted-foreground">Owners.</span>
              </h1>
            ...
            {(isSuperAdmin || user?.role === "project_manager") && (
              <Link href="/project-developers/register">
                <Button className="rounded-none bg-secondary hover:bg-emerald-900 text-[10px] font-bold uppercase tracking-widest transition-colors h-12 px-6">
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
          <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            ...
            <input
              ...
              className="w-32 bg-transparent border-none border-b-2 border-border py-2 text-[10px] font-bold uppercase tracking-widest text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-slate-900 transition-colors rounded-none"
            />
        ...
        {/* ── Institutional Stats ── */}
        {!isLoading && data && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-slate-200 border border-border mb-8">
            {[
        ...
        {isLoading ? (
          ...
          <div className="py-24 border border-border bg-red-50 flex flex-col items-center justify-center text-center">
            ...
            <Button
              onClick={() => refetch()}
              className="rounded-none bg-secondary text-white font-bold text-[10px] uppercase tracking-widest px-6"
            >
              Retry Connection
            </Button>
          </div>
            ...
            <Table>
              <TableHeader className="bg-muted">
                <TableRow className="border-b-2 border-slate-900 hover:bg-muted">
                  <TableHead className="text-[10px] font-bold uppercase tracking-[0.2em] text-foreground h-14">
                    Entity Name
                  </TableHead>
                  <TableHead className="text-[10px] font-bold uppercase tracking-[0.2em] text-foreground h-14">
                    Registry Code
                  ...
                  <TableHead className="text-[10px] font-bold uppercase tracking-[0.2em] text-foreground h-14 text-right">
                    Details
                  </TableHead>
                </TableRow>
              </TableHeader>
                ...
                {owners.map((owner) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="font-mono text-[11px] text-muted-foreground uppercase tracking-widest py-4">
                        {owner.code}
                      </TableCell>
                      <TableCell className="py-4">
                      ...
                      <TableCell className="py-4 text-right">
                        <Link href={`/project-developers/${owner.userId}`}>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="rounded-none text-muted-foreground hover:text-foreground hover:bg-slate-100"
                          >
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {owners.map((owner) => {
              ...
              return (
                <Link
                  key={owner.id}
                  href={`/project-developers/${owner.userId}`}
                  className="block group"
                >
                  <div className="border border-border bg-white hover:border-slate-900 transition-colors h-full flex flex-col">
                    <div className="p-6 flex-1">
                      ...
                      <h3 className="font-sans text-xl text-foreground leading-tight mb-1 group-hover:text-emerald-800 transition-colors">
                        {owner.firstName} {owner.lastName}
                      </h3>
