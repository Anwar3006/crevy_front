...
export function DataTable({ columns, data, searchKey, placeholder = "Search..." }) {
  const table = useReactTable({ data, columns, getCoreRowModel: getCoreRowModel(), getPaginationRowModel: getPaginationRowModel() });
  return (
    <div className="space-y-4">
      <div className="relative max-w-sm">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <Input placeholder={placeholder} className="pl-10 rounded-none border-border bg-background" />
      </div>
          ...
          <TableHeader className="bg-muted/50">
            {table.getHeaderGroups().map(hg => (
              <TableRow key={hg.id} className="border-b-2 border-foreground">
                {hg.headers.map(h => (
                  <TableHead key={h.id} className="text-[10px] font-bold uppercase tracking-widest text-foreground h-14">
